...
public class AttackOptionBase
{
    ...
    protected bool AutoAttackTimer(float _coolTime)
    {
        timer += Time.deltaTime;

        if (timer > _coolTime) return true; else return false;
    }
    ...
    protected Vector3 AutoAttackVector(Transform _playerTrans, List<EnemyBase> _enemyList, float _maxDistance)
    {
        ...
        Vector3 rangeVec = Quaternion.Euler(0, r, 0) * _playerTrans.position + _playerTrans.up * 20;
